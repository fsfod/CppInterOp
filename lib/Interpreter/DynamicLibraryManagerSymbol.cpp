...
#define DEBUG_TYPE "Dyld::ContainsSymbol:"
    ...
    if (m_UseBloomFilter) {
      ...
      LLVM_DEBUG(dbgs() << "Dyld::ContainsSymbol: BloomFilter: Symbol <" << mangledName.str() << "> May exist."
                        << " Search for it. ");
    }
...
#define DEBUG_TYPE "Dyld:"
    ...
    if (!ObjF) {
      LLVM_DEBUG(dbgs() << "[DyLD] Failed to read object file "
                        << FileName << "\n");
      return true;
    }
...
#define DEBUG_TYPE "Dyld:"
    ...
    for (auto const &item : m_BasePaths.m_Paths) {
      LLVM_DEBUG(dbgs() << "Dyld: - m_BasePaths[" << x++ << "]:"
                        << &item << ": " << item << "\n");
    }
    LLVM_DEBUG(dbgs() << "---\n");
    x = 0;
    for (auto const &item : m_Libraries.GetLibraries()) {
      LLVM_DEBUG(dbgs() << "Dyld: - m_Libraries[" << x++ << "]:"
                        << &item << ": " << item->m_Path << ", "
                        << item->m_LibName << "\n");
    }
    x = 0;
    for (auto const &item : m_SysLibraries.GetLibraries()) {
      LLVM_DEBUG(dbgs() << "Dyld: - m_SysLibraries[" << x++ << "]:"
                        << &item << ": " << item->m_Path << ", "
                        << item->m_LibName << "\n");
    }
#undef  DEBUG_TYPE
...
#define DEBUG_TYPE "Dyld:searchLibrariesForSymbol:"
    ...
    if (m_FirstRun) {
      ...
      LLVM_DEBUG(dbgs() << "Dyld::searchLibrariesForSymbol: After first ScanForLibraries\n");
      dumpDebugInfo();
    }
    ...
    for (const LibraryPath* P : m_Libraries.GetLibraries()) {
      if (ContainsSymbol(P, mangledName, /*ignore*/
        ...
        LLVM_DEBUG(dbgs() << "Dyld::ResolveSymbol: Search found match in [user lib]: "
                          << P->GetFullName() << "!\n");
    ...
    if (m_FirstRunSysLib) {
      ...
      LLVM_DEBUG(dbgs() << "Dyld::searchLibrariesForSymbol: After first system ScanForLibraries\n");
      dumpDebugInfo();
    }
    ...
    for (const LibraryPath* P : m_SysLibraries.GetLibraries()) {
      if (ContainsSymbol(P, mangledName, /*ignore*/
        ...
        LLVM_DEBUG(dbgs() << "Dyld::ResolveSymbol: Search found match in [system lib]: "
                          << P->GetFullName() << "!\n");
